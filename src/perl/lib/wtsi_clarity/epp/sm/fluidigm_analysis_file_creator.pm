...
sub _build__sample_plate {
  my $self = shift;
  return $self->plate_io_map->[0]->{'source_plate'};
}

sub _build__barcode {
  my $self = shift;
  return $self->plate_io_map_barcodes->[0]->{'source_plate'};
}
...
sub _build__samples {
  ...
  for (1..$self->_container_size->{'x_dimension'}) {
    foreach my $column ('A'..$big_row) {
      ...
      my $artifact_list = $self->input_artifacts
...
sub _build__container_size {
  ...
  my $container = $self->fetch_and_parse(join q{/}, $self->config->clarity_api->{'base_uri'}, 'containers', $self->_sample_plate);
